...
#pragma warning restore CA1822
    ...
    {
        var lines = context.DailyDistrictConditions
            .Where(definition => definition.District != context.World.CurrentDistrict)
        ...
        if (lines.Count == 0)
        {
            lines =
            [
                "No other district bulletins are active right now."
            ];
        }
    ...
    private static GameStatusPage BuildDebtPage(GameStatusContext context)
    {
        var lines = new List<string>
        {
            ...
            context.UnpaidRentDays == 0
                ? "Rent is paid up today."
                : "Rent is overdue and keeps landlord pressure active.",
            context.UnpaidRentDays >= Slums.Core.Expenses.RentState.EvictionThreshold
                ? "Eviction risk is active."
                : "Eviction risk is not active yet."
        };
