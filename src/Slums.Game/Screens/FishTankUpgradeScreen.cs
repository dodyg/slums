...
internal sealed class FishTankUpgradeScreen : ScreenSurface
{
    private const int ListX = 2;
    private const int ListY = 6;
    private const int DetailX = 36;
    private readonly FishTankUpgradeCommand _command = new();
    private readonly FishTankUpgradeMenuContext _context;
    ...
    private readonly List<FishTankUpgradeMenuStatus> _statuses;
    private int _selectedIndex;

    public FishTankUpgradeScreen(int width, int height, GameSession gameState, FishTankUpgradeMenuContext context, HouseholdAssetsScreen parentScreen, GameScreen rootParentScreen)
        : base(width, height)
    {
        ...
        _statuses = new FishTankUpgradeMenuQuery().GetStatuses(context).ToList();
        IsFocused = true;
        UseMouse = true;
    ...
    public override void Render(TimeSpan delta)
    {
        ...
        Surface.Print(ListX, 2, "=== Fish Tank Upgrades ===", Color.Cyan);
        Surface.Print(ListX, 3, $"Money: {_context.Money} LE | Week {_context.CurrentWeek}", Color.Gray);
        Surface.Print(ListX, 4, $"Care this week: {(_context.FishTank.IsUpkeepPaidForWeek(_context.CurrentWeek) ? "covered" : "due")}", Color.Gray);
    ...
    private void RenderSelectedDetails()
    {
        ...
        y++;
        Surface.Print(DetailX, y++, $"Fish Tank: {_context.Definition.Name}", Color.White);
