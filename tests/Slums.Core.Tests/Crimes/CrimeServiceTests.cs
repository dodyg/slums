...
internal sealed class CrimeServiceTests
{
    ...
    public void AttemptCrime_ShouldSucceedFrequently_WhenRiskAndPressureAreLow()
    {
        ...
        for (var i = 0; i < 200; i++)
        {
            if (CrimeService.AttemptCrime(attempt, player, 0, random).Success)
            {
                successes++;
            }
        }
    ...
    public void AttemptCrime_ShouldDetectFrequently_WhenRiskAndPressureAreHigh()
    {
        ...
        for (var i = 0; i < 200; i++)
        {
            if (CrimeService.AttemptCrime(attempt, player, 90, random).Detected)
            {
                detections++;
            }
        }
    ...
    public void PreviewCrime_ShouldReflectStreetSmartsAndPressureThresholds()
    {
        ...
        var preview = CrimeService.PreviewCrime(attempt, player, policePressure: 60);
    ...
    {
        var unskilled = new PlayerCharacter(TestContent.Catalog);
        var hacker = new PlayerCharacter(TestContent.Catalog);
        ...
        var baseline = CrimeService.PreviewCrime(attempt, unskilled, policePressure: 0);
        var boosted = CrimeService.PreviewCrime(attempt, hacker, policePressure: 0);
